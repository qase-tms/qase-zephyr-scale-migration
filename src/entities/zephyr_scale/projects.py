...
class Projects:
    ...
    async def _import_projects_async(self) -> Mappings:
        ...
        await asyncio.gather(*[
            self._import_project(i, project, total)
    ...
    def _validate_selection(self):
        """Refuse to run on an ambiguous project selection.

        Migrating every project is something you opt into with ``import_all``,
        never something an empty ``projects.import`` does by default. Otherwise
        a typo in the key list silently migrates the whole tenant.
        """
        import_all = bool(self.config.get("projects.import_all"))
        selected = [str(p).strip() for p in (self.config.get("projects.import") or []) if str(p).strip()]
        excluded = [str(p).strip() for p in (self.config.get("projects.exclude") or []) if str(p).strip()]

        if not import_all and not selected:
            raise ValueError(
                "No projects selected: 'projects.import_all' is false and 'projects.import' "
                "is empty. List the Jira project keys to migrate, or set "
                "'projects.import_all': true to migrate every Zephyr Scale project."
            )

        overlap = sorted(set(selected) & set(excluded))
        if overlap:
            raise ValueError(
                f"Project key(s) {overlap} appear in both 'projects.import' and "
                f"'projects.exclude'. Remove them from one side."
            )

    def _check_import(self, key: str, name: str) -> bool:
        """Should this Zephyr project be migrated? Exclusion always wins."""
        import_all = bool(self.config.get("projects.import_all"))
        selected = [str(p).strip() for p in (self.config.get("projects.import") or []) if str(p).strip()]
        excluded = [str(p).strip() for p in (self.config.get("projects.exclude") or []) if str(p).strip()]
        ...
        if import_all:
            return True
        return key in selected or name in selected

    async def _import_project(self, i: int, project: dict, total: int):
        key = project.get("key") or ""
        name = project.get("name") or key

        if not self._check_import(key, name):
