...
# Retry and step-shape behaviour are not configuration: a customer has no basis on
# which to tune them, and exposing them invites a worse run.
_RETRY_MAX_ATTEMPTS = 5
_RETRY_BACKOFF_SEC = 2.0

# The Qase SDK generates the wrong JSON for case-level attachments, so the bulk
# body is built by hand. These control that hand-built body. They are constants
# rather than config because no customer could reasonably choose a value.
_BULK_STEPS_TYPE = "classic"
_BULK_INJECT_STEPS_TYPE_ALWAYS = True
_BULK_INJECT_STEPS_TYPE_WITH_ATTACHMENTS = True
_BULK_HYDRATE_CASE_IDS = True
...
class QaseService:
    ...
    def get_all_users(self, limit=100):
        offset = 0
        while True:
            result = self._get_users(limit, offset) or []
            yield result
            offset += limit
            if len(result) < limit:
                break
    ...
    def resolve_user_id(self, value):
        ...
        raise ValueError(
            ...
            f"Check the address, or that the user has accepted their invitation to the workspace."
        )
    ...
    def create_custom_field(self, data) -> int:
        try:
            ...
            if not api_response.status:
                self.logger.log('Error creating custom field: ' + data['title'], 'error')
            else:
    ...
    def create_configuration_group(self, project_code, title):
        try:
            ...
            if not api_response.status:
                self.logger.log('Error creating configuration group: ' + title, 'error')
            else:
    ...
    def create_configuration(self, project_code, title, group_id):
        try:
            ...
            if not api_response.status:
                self.logger.log('Error creating configuration: ' + title, 'error')
            else:
    ...
    def _build_case_bulk_json_body(self, chunk: list) -> dict:
        ...
        steps_type = _BULK_STEPS_TYPE
        inject_with_att = bool(
            _BULK_INJECT_STEPS_TYPE_WITH_ATTACHMENTS
        )
        inject_always = bool(
            _BULK_INJECT_STEPS_TYPE_ALWAYS
        )
        n_with_att = 0
    ...
    def upload_attachment(self, code, attachment_data):
        ...
        try:
            ...
            self.logger.log(f'Exception when calling AttachmentsApi->upload_attachment: {e}', 'error')
        return None
