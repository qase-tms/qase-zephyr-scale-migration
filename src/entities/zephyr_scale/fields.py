...
class Fields:
    ...
    async def _import_fields_async(self) -> Mappings:
        ...
        if system_fields:
            ...
            self.logger.log("[Fields] No system fields returned from Qase", "warn")
    ...
    async def _sync_custom_fields(self):
        ...
        for proj in self.mappings.projects:
            ...
            try:
                ...
                self.logger.log(
                    f"[Fields] Failed to fetch ZS custom fields for {zephyr_key}: {e}", "warn"
                )
        ...
        if not zs_fields_by_name:
            ...
            for proj in self.mappings.projects:
                ...
                try:
                    ...
                    self.logger.log(
                        f"[Fields] Failed to scan TCs for {zephyr_key} CF discovery: {e}", "warn"
                    )
        ...
        try:
            ...
            self.logger.log(
                f"[Fields] Failed to fetch existing Qase custom fields: {e}", "warn"
            )
        ...
        for name, zs_cf in zs_fields_by_name.items():
            ...
            if name_lower in existing_qase_cfs:
                ...
                if qase_cf_id:
                    ...
                    self.logger.log(f"[Fields] Failed to create Qase CF '{name}'", "warn")
                    continue
